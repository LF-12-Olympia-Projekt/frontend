...
export function AllSportsGrid({ sports, locale, eventsLabel, medalsLabel }: AllSportsGridProps) {
  return (
          ...
          <Link key={sport.id} href={`/${locale}/sports/${sport.id}`}>
            <Card className="group h-full border-0 bg-[#1a1a1a] shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <CardContent className="flex flex-col gap-4 p-6">
                {/* Icon */}
                <div className={`flex h-16 w-16 items-center justify-center rounded-2xl ${iconConfig.color}`}>
