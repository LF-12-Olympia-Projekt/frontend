...
export function NationsGrid() {
  ...
  useEffect(() => {
    Promise.all([getMedalStandings({ limit: 100 }), getCountries()])
      .then(([medalData, allCountries]) => {
        const medalMap = new Map<string, MedalStanding>()
        medalData.data.forEach((ms) => {
          medalMap.set(ms.country.countryCode, ms)
        })
        const mapped = allCountries.map((c) => {
          const ms = medalMap.get(c.countryCode)
          return {
            code: c.countryCode || c.countryAbbr,
            name: c.name,
            gold: ms?.gold ?? 0,
            silver: ms?.silver ?? 0,
            bronze: ms?.bronze ?? 0,
            total: (ms?.gold ?? 0) + (ms?.silver ?? 0) + (ms?.bronze ?? 0),
          }
        })
        setCountries(mapped)
      })
                  ...
                  <div className="flex-1">
                    ...
                    {country.total > 0 && (
                      <p className="text-sm text-muted-foreground">
                        {country.total} {medalTable.total || "Medals"}
                      </p>
                    )}
                  </div>
                </div>
